...
def actualizar_por_hacer(
    ...
    try:
        ...
        filas_afectadas = repository.update_por_hacer(
            cursor,
            item_id,
            nombre=body.nombre,
...
def borrar_por_hacer(item_id: int, _: UsuarioActual = Depends(get_current_user)) -> None:
    db_conn = get_connection()
    try:
        cursor = db_conn.cursor()
        filas_afectadas = repository.delete_por_hacer(cursor, item_id)
        if filas_afectadas == 0:
            db_conn.rollback()
            raise HTTPException(status_code=404, detail="Ítem no encontrado")
        db_conn.commit()
