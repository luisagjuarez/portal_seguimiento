...
import { crearPlantillaSolicitud, fetchMiembrosEquipo, fetchTiposSolicitud } from "../api.js";

export default function CrearPlantillaSolicitudFormulario({ onCreada, onCancelar }) {
  ...
  const [ordenPrioridadDefault, setOrdenPrioridadDefault] = useState(3);
  ...
  const enviar = async (event) => {
    ...
    setError(null);
    try {
      const creada = await crearPlantillaSolicitud({
        nombre: nombre.trim(),
      ...
      setError(err.message || "No se pudo crear la plantilla. Intenta de nuevo.");
    } finally {
      setEnviando(false);
    }
  };
