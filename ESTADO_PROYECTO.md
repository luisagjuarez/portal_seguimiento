...
[~] Fase 1.26 (extraoficial) — Tablero de tareas: nuevo rango "Fecha inicio"/"Fecha fin" que delimita `fecha_fin` (default: últimos 8 días hasta hoy), y filtro de responsable multi-selectivo agrupado por área/perfil (checkbox "seleccionar todos" por área con estado indeterminado); backend `/api/tareas` acepta `responsable_id` repetido + `desde`/`hasta`, `/api/miembros-equipo` expone `perfil` — implementado y verificado por 220 tests de backend + curl e2e contra la BD real local (2026-09-07); falta desplegar a TEST y la pasada visual del usuario
```
